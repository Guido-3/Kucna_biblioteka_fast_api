...
def create_autor(autor: schemas.Autor, db: Session = Depends(get_db)):
    return autor_crud.create_autor(db=db, autor=autor)

@router.get("/autori/{autor_id}", response_model=schemas.Autor)
def get_autor(autor_id, db: Session = Depends(get_db)):
...
def update_autor(autor_id: int, autor: schemas.Autor, db: Session = Depends(get_db)):
